/*
 ...
 This version of the demo uses functions that take the actual data 
 types like struct Point*, and uses function pointer casts to 
 suppress the compiler's warning of incompatible function
 pointer types.
 
 Use a simple program like this as a "test harness" to systematically 
 test your Ordered_container functions, starting with the simplest 
 and most basic. Be sure that you test them all!
 */
...
/* these functions all use actual data type parameters */

int compare_Points(const struct Point* ptr1, const struct Point* ptr2);
...
int main(void)
{
  ...
  /* function pointer cast to suppress warning/error about 
   incompatible function pointer types */
  container = OC_create_container((int(*)(const void*, const void*))compare_Points);
  /* we could also use the typedef for the function pointer type:
   container = OC_create_container((OC_comp_fp_t)compare_Points);
   */
  ...
  /* function pointer cast to suppress warning/error about 
   incompatible function pointer types */
  OC_apply(container, (void(*)(void*))print_Point);
  /* we could also use the typedef for the function pointer type:
  ...
  // What is this for???????????????????????????????????????????????
  {
    ...
    OC_apply_arg(container, (void(*)(void*, void*))demo_apply_func, (void *)&odd_or_even_value);
    odd_or_even_value = 3;
    OC_apply_arg(container, (OC_apply_arg_fp_t)demo_apply_func, (void *)&odd_or_even_value);
  }
...
/* this apply-function uses the arg parameter to make a decision 
 about what to output for each item */
void demo_apply_func(struct Point* data_ptr, int* arg)
{
  if(*arg % 2)  /* is the arg even or odd? */
...
/* search the container for the probe item; if found output it and 
 then delete it */
void find_and_remove(struct Ordered_container * container, struct Point probe)
{
  printf("\nsearch for (%d, %d):\n", probe.x, probe.y);
