...
int main ()
{
  ...
  
  {
    ...
    /* Technically, initializing a variable has to be done with something of the 
     same type. For example, 
     int i = 3.14;
     requires that the compiler convert 3.14 into an integer, and create the 
     machine instructions to initialize i with that integer value.  
     Compare this with:
     String a = "Hello";
     where the initial value is a C-string, not a String. The compiler has to 
     first convert the C-string into a String. It does this by taking
     advantage of the fact that a constructor that takes a single argument of a 
     particular other type is a way to convert that type into the class type. 
     So the compiler first constructs an unnamed String using the C-string "Hello" 
     as a constructor parameter, and then copy or move constructs String a from it.
     
     However, almost all compilers can short-cut this and simply initialize 
     String a using "Hello" as a constructor parameter, leaving out the unnamed 
     String construction and copy/move construction from it. Most compilers are 
     agressive about eliminating these logically unnecessary constructor calls, 
     even if it can change the side effects produced by the code.
     
     This is called "constructor elision" - from "elide", meaning "to omit", 
     from the Latin "elidere", meaning "to crush out".
     */
    String a = "Hello";
...
/*
 In addition to eliding constructor calls in initialization, almost all 
 compilers will also perform the Return Value Optimization (RVO) to eliminate 
 the copy or move construction of the returned value; instead of building the 
 return value in a local variable and then copying it out into the unnamed returned
 variable in the the caller's stack space, the compiler builds the local variable 
 in that same returned variable space, thereby saving a copy or move construction. 
...
/*
 But in the type of function shown below, where the returned value is the 
 same variable as the input parameter, The RVO cannot be done! The variable s 
 can't be placed both in the local stack space of the parameter and in the 
 caller's stack space for the returned value. This forces the compiler to call 
 a copy or move constructor to get the returned value into the returned variable space.
 */
...
/* Output from program - NOTICE: Different compilers may produce different results
 due to different compiler optimizations, especially by "eliding" or eliminating 
 constructor calls that are logically redundant even if it changes the side 
 effects (like outputting messages). This output was produced with gcc 5.1.0 on 
 CAEN Linux
 
 -bash-4.2$ g++ -pedantic-errors -std=c++14 String_demo3.cpp String.cpp
 -bash-4.2$ ./a.out
...
/*
 ...
 gcc compiler option which requires the compiler to NOT eliminate any constructor calls.
 There are many more constructor calls, each of which consumes run time, but this has
 all the constructor calls "by the book."
 
 -bash-4.2$ g++ -pedantic-errors -std=c++14 -Wall -fno-elide-constructors String_demo3.cpp String.cpp p2_globals.cpp Utility.cpp
 -bash-4.2$ ./a.out
 0 Strings using 0 bytes
